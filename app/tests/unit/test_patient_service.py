...
class TestPatientService:
    ...
    def test_update_patient_logs_changes(self, service, mock_db):
        patient = MagicMock(id=1, first_name="John", last_name="Doe")
        service.repository.get_by_id = MagicMock(return_value=patient)
        service.repository.get_detailed_by_id = MagicMock(return_value=patient)
        
        # Mock snapshots
        service._build_demographic_snapshot = MagicMock(return_value={"first_name": "John"})
        service._calculate_changed_fields = MagicMock(return_value=["first_name"])
        
        payload = PatientUpdateSchema(first_name="Johnny")
