# app/tests/integration/test_reimbursement_routes.py
from __future__ import annotations

"""
Integration tests for /api/v1/reimbursements routes.
"""
...
def _payload(staff_profile_id: int = 1) -> dict:
    return {
        ...
        "receipt_url": "https://example.com/receipts/r1.png",
    }


class TestReimbursementRoutes:
    def test_list_returns_envelope(self, client, auth_header):
        res = client.get("/api/v1/reimbursements", headers=auth_header)
    ...
    def test_list_supports_pagination(self, client, auth_header):
        res = client.get(
            "/api/v1/reimbursements?skip=0&limit=5", headers=auth_header
        )
        assert res.status_code == 200
    ...
    def test_create_validates_missing_required(self, client, auth_header):
        ...
        bad.pop("amount")
        res = client.post(
            "/api/v1/reimbursements", json=bad, headers=auth_header
        )
        assert res.status_code == 422

    def test_create_rejects_negative_amount(self, client, auth_header):
        bad = _payload()
        bad["amount"] = "-10.00"
        res = client.post(
            "/api/v1/reimbursements", json=bad, headers=auth_header
        )
        # The schema has max_digits/decimal_places but doesn't enforce >0;
        # service may accept or reject — we just want a clean response.
    ...
    def test_get_unknown_returns_404(self, client, auth_header):
        res = client.get(
            "/api/v1/reimbursements/999999", headers=auth_header
        )
        assert res.status_code == 404

    def test_update_unknown_returns_404(self, client, auth_header):
        res = client.put(
            "/api/v1/reimbursements/999999",
    ...
    def test_submit_unknown_returns_4xx(self, client, auth_header):
        res = client.post(
            "/api/v1/reimbursements/999999/submit",
...
class TestReimbursementRoutesAuth:
    def test_anonymous_list_returns_401(self, client):
        assert client.get("/api/v1/reimbursements").status_code == 401

    def test_anonymous_create_returns_401(self, client):
        assert client.post("/api/v1/reimbursements", json=_payload()).status_code == 401

    def test_anonymous_update_returns_401(self, client):
        assert client.put("/api/v1/reimbursements/1", json={}).status_code == 401

    def test_anonymous_delete_returns_401(self, client):
        assert client.delete("/api/v1/reimbursements/1").status_code == 401
